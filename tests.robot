...
${VALID_USERNAME}   Modern_qwerty
${VALID_PASSWORD}   Modern_qwerty
${BROWSER}          Chrome
...
Login With Credentials
    ...
    Wait Until Element Is Visible    name=username    timeout=15
    Input Text    name=username    ${username}
    Input Password    name=password    ${password}
    Click Button    css=input[value='Log In']

Verify Accounts Overview Page Loaded
    Wait Until Page Contains    Accounts Overview    timeout=15
    Wait Until Element Is Visible    id=accountTable    timeout=15

Get Account Balance
    Wait Until Element Is Visible    css=#accountTable tr:first-child td:nth-child(2)    timeout=15
